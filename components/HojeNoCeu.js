...
/**
 * "Hoje no Amigos do Céu" — o ritual diário da home: a celebração, a figurinha e a oração.
 *
...
function HojeNaIgreja({ celebracao, santos, oracao }) {
  ...
  return (
    <div className="p-6 flex flex-col">
      ...
      <div className="mt-auto pt-5 flex flex-wrap gap-2">
        ...
        {oracao && (
          <Link href={oracao.href} className={botaoSecundario}>
            Rezar
          </Link>
        )}
      </div>
...
function FigurinhaDeHoje({ figurinha, ehSantoDoDia }) {
  const { loaded, figurinhaDoDiaJaResgatada, resgatarFigurinhaDoDia } = useAlbum()
  const [recemColada, setRecemColada] = useState(null) // true: nova | false: já tinha
  ...
  const receber = () => {
    const { ok, nova } = resgatarFigurinhaDoDia(figurinha)
    if (ok) setRecemColada(nova)
  }
    ...
    <div className="p-6 flex gap-5 items-center md:flex-col md:items-start">
      <div className="w-24 sm:w-28 shrink-0 md:order-2 md:mx-auto">
        {aberta ? (
          <motion.div
            key="face"
            initial={recemColada !== null && !reduzir ? { rotateY: 90, opacity: 0 } : false}
        ...
        {aberta ? (
          <>
            ...
            <p className="mt-1 text-sm text-neutral-400">
              {recemColada === false ? 'Já estava no seu álbum.' : 'Colada no seu álbum.'} Amanhã tem outra.
            </p>
          ...
          <>
            <p className="mt-3 text-sm text-neutral-300 leading-relaxed">
              {ehSantoDoDia ? 'Hoje a Igreja celebra um santo que está no álbum.' : 'Todo dia uma figurinha diferente espera por você.'}
            </p>
            <noscript>
              <p className="mt-1 text-sm text-neutral-400">Figurinha de hoje: {figurinha.nome}</p>
            </noscript>
            <button type="button" onClick={receber} className={`mt-4 ${botaoPrimario}`}>
              Receber
            </button>
          </>
        )}
  ...
  return (
    <div className="p-6 flex flex-col">
      ...
      <div className="mt-auto pt-5">
        <Link href={oracao.href} className={botaoPrimario}>
          Rezar agora
        </Link>
      </div>
    </div>
...
export default function HojeNoCeu({ dias, hojeServidor }) {
  ...
  return (
    <section aria-labelledby="hoje-titulo" className="px-4 pb-10">
      <div className="max-w-6xl mx-auto rounded-3xl border border-cosmic-border bg-gradient-to-br from-cosmic-surface via-cosmic-surface/80 to-cosmic-bg shadow-[0_30px_60px_-30px_rgba(0,0,0,.8)] overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-cosmic-border md:min-h-[17rem]">
          <HojeNaIgreja celebracao={dia.celebracao} santos={dia.santos} oracao={dia.oracao} />
          <FigurinhaDeHoje figurinha={dia.figurinha} ehSantoDoDia={dia.figurinhaEhSantoDoDia} />
          <OracaoDeHoje oracao={dia.oracao} />
        </div>
      </div>
    </section>
  )
