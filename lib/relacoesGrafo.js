...
export function construirGrafo(dados) {
  ...
  // santo↔santo já cadastrado em santos.json (legado)
  const santoSanto = new Set()
  dados.santos.forEach((s) =>
    (s.relacionamentos || []).forEach((r) => {
      const alvo = `santo:${r.santoSlug}`
      if (!entidades.has(alvo)) erros.push(`santos.json: ${s.slug}.relacionamentos aponta para santo inexistente "${r.santoSlug}"`)
      santoSanto.add(chaveAresta(`santo:${s.slug}`, alvo))
    })
  )
