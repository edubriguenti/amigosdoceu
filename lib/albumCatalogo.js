...
/** Id (chave de persistência) da figurinha de uma entidade. Único lugar que define o formato. */
export function idFigurinha(tipo, slug) {
  return `${tipo}:${slug}`
}
...
export function construirCatalogo({ album, santos, aparicoes, igrejas, vidaCristo, calendario }) {
  ...
  TIPOS.forEach((tipo) => {
    ;(fontesPorTipo[tipo] || []).forEach((ent) => {
      ...
      const id = idFigurinha(tipo, ent.slug)
      if (entidades.has(id)) {
